...
if (isset($_FILES['profile_photo']) && $_FILES['profile_photo']['error'] === UPLOAD_ERR_OK) {
    ...
    if (in_array($file_ext, $allowed_types)) {
        ...
        $message = "<div class='p-4 mb-6 text-sm text-red-300 bg-red-900/30 rounded-lg border border-red-800'>Only JPG, PNG, and GIF images are allowed.</div>";
            ...
            $sql = "UPDATE users SET name = ?, email = ?, contact = ?, address = ?, profile_photo = ? WHERE user_id = ?";
            
            $stmt = $conn->prepare($sql);
if ($stmt) {
    $stmt->bind_param("sssssi", $name, $email, $contact, $address, $profile_photo, $user_id);
                if ($stmt->execute()) {
            ...
            $message = "<div class='p-4 mb-6 text-sm text-red-300 bg-red-900/30 rounded-lg border border-red-800'>Name, email and contact are required fields.</div>";
        }
    }
    ...
    if (isset($_POST['toggle_2fa'])) {
        ...
        $status = $_SESSION['2fa_enabled'] ? 'enabled' : 'disabled';
        $message = "<div class='p-4 mb-6 text-sm text-green-300 bg-green-900/30 rounded-lg border border-green-800'>Two-factor authentication has been $status.</div>";
    }
}
...
<head>
    ...
    <title>DeliverDash | Profile</title>
            ...
            <div class="w-full md:w-72 flex-shrink-0">
                <div class="bg-gradient-to-br from-dark-800 to-dark-900 rounded-xl shadow-xl border border-gray-800/50 overflow-hidden">
                    ...
                    <div class="border-t border-gray-800/50 px-2 py-3">
                        ...
                        <a href="user_settings.php" class="nav-link block px-6 py-3 text-sm font-medium rounded-lg mx-2">
                    ...
                    <div class="profile-section rounded-xl shadow-lg p-6">
                        <div class="flex items-center mb-6">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500">Update your basic profile details</p>
                            </div>
                        </div>
...
<div class="relative">
    ...
    <input 
        ...
        placeholder="Enter your phone number">
                    ...
                    <div class="flex flex-col sm:flex-row justify-end gap-4 pt-4">
                        <a href="../users/user_dashboard.php" class="order-2 sm:order-1">
                            <button type="button" class="w-full sm:w-auto px-6 py-2.5 rounded-lg font-medium border border-gray-700 hover:bg-dark-700 transition flex items-center justify-center">
                                <i class="fas fa-times mr-2"></i> Cancel
                            </button>
                        </a>
                        <button type="submit" class="order-1 sm:order-2 w-full sm:w-auto bg-gradient-to-r from-primary-500 to-primary-600 text-white py-2.5 px-6 rounded-lg font-medium hover:from-primary-600 hover:to-primary-700 transition duration-300 shadow-lg hover:shadow-glow flex items-center justify-center">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" id="new_username" name="new_username" required minlength="4"
                                   class="w-full pl-10 pr-4 py-2.5 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                                   placeholder="Enter new username (min 4 characters)" value="<?php echo htmlspecialchars($user['username']); ?>">
                        </div>
                    </div>
    ...
    let currentUser = {
        ...
        profilePhoto: "<?php echo $user['profile_photo'] ?? ''; ?>"
    };
    ...
    document.getElementById('profile_photo').addEventListener('change', function(e) {
        ...
        reader.onload = function(event) {
            ...
            const navProfileImage = document.querySelector('#user-menu img');
            const navProfileInitials = document.querySelector('#user-menu div');
            ...
            if (navProfileImage) {
                ...
                document.getElementById('user-menu').insertBefore(newNavImg, navProfileInitials);
            }
        };
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                currentUser.profilePhoto = data.profile_photo;
            } else {
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                if (data.name) {
                    ...
                    document.querySelector('.avatar + h2').textContent = currentUser.name;
                }
            } else {
                showMessage('error', data.message);
            }
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                if (data.username) {
                    ...
                    document.querySelector('.bg-dark-700/50 span').textContent = currentUser.username;
                }
                closeModal();
            } else {
                showMessage('error', data.message);
    ...
    // Handle 2FA form submission with AJAX
    document.getElementById('2faForm').addEventListener('submit', function(e) {
        ...
        .then(data => {
            ...
            if (data.success) {
                showMessage('success', data.message);
                const statusElement = document.querySelectorAll('.bg-dark-700/50 span')[2];
                const buttonElement = document.querySelectorAll('.bg-dark-700/50 button')[2];
                
                if (statusElement && buttonElement) {
                    statusElement.textContent = data.enabled ? 'Enabled' : 'Disabled';
                    buttonElement.innerHTML = data.enabled ? '<i class="fas fa-toggle-on mr-1"></i> Disable' : '<i class="fas fa-toggle-off mr-1"></i> Enable';
                }
                closeModal();
            } else {
                showMessage('error', data.message);
        ...
        .catch(error => {
            hideLoading();
            showMessage('error', 'An error occurred while updating two-factor authentication.');
            console.error('Error:', error);
        });
    });
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('input[type="checkbox"]').forEach(checkbox => {
            checkbox.addEventListener('change', function() {
                // You can add AJAX here if you want to save notification preferences
                console.log(`Notification ${this.id} ${this.checked ? 'enabled' : 'disabled'}`);
            });
        });
    });
</script>
